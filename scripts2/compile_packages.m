...
function compile_packages()
    ...
    packages2Dir = fullfile(projectRoot, 'packages2');
    ...
    for i = 1:length(dirPaths)
        ...
        yamlPath = fullfile(packages2Dir, packageName, 'prepare.yaml');
        ...
        % Read YAML file
        yamlData = readYamlFile(yamlPath);
        if isempty(yamlData)
            fprintf('\n%s: Could not read prepare.yaml - skipping\n', dirName);
            continue;
        end
        
        % Check if any build matches current BUILD_TYPE and has compile_script
        compileScript = '';
        if isfield(yamlData, 'builds')
...
function yamlData = readYamlFile(filepath)
    % Simple YAML reader for our prepare.yaml files
    % This is a basic implementation that handles our specific YAML structure
    yamlData = struct();
    ...
    try
        ...
        for i = 1:length(lines)
            ...
            if startsWith(line, 'builds:')
                inBuilds = true;
                continue;
            end
            
            if inBuilds
                if startsWith(line, '- build_type:')
                    % Save previous build if exists
                    if ~isempty(fieldnames(currentBuild))
                        builds{end+1} = currentBuild;
                    end
                    % Start new build
                    currentBuild = struct();
                    parts = strsplit(line, ':');
                    if length(parts) >= 2
                        currentBuild.build_type = strtrim(parts{2});
                    end
                elseif startsWith(line, 'compile_script:')
                    parts = strsplit(line, ':');
                    if length(parts) >= 2
                        currentBuild.compile_script = strtrim(parts{2});
                    end
        ...
        % Add last build if we're still in builds section
        if inBuilds && ~isempty(fieldnames(currentBuild))
            builds{end+1} = currentBuild;
        end
        ...
        fprintf('Error reading YAML file: %s\n', ME.message);
        yamlData = struct();
    end
end